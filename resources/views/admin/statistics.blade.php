...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h1 class="panel-title pull-left">Статистика (месяцы)</h1>
        </div>
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h3 class="panel-title pull-left">Статистика по задачам</h3>
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h3 class="panel-title pull-left">Статистика по доходам</h3>
            @include('admin.blocks._heading_elements_block')
        </div>
        <div class="panel-body">
            @include('admin.blocks._chart_container_block', [
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h3 class="panel-title pull-left">Статистика по годам</h3>
            @include('admin.blocks._heading_elements_block')
        </div>
        <div class="panel-body">
            @include('admin.blocks._chart_container_block', [
    ...
    @foreach ($data['done_tasks'] as $task)
        ...
        <script>window.statisticsData[1].dataHorAxis[0].data[parseInt("{{ $month - $data['min_val'] }}")] += parseInt("{{ Helper::calculateOverTaskVal($task, true, true, true, true) }}");</script>
    @endforeach

    @foreach ($data['wait_tasks'] as $task)
        <script>window.statisticsData[1].dataHorAxis[1].data[parseInt("{{ (int)date($precision,$task->completion_time)-$data['min_val'] }}")] += parseInt("{{ Helper::calculateOverTaskVal($task, true, true, true, true) }}");</script>
    @endforeach
