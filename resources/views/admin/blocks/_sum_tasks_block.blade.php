<div class="total-line text-grey-300">
    Итого {{ isset($addTotalDesk) ? '('.$addTotalDesk.')' : '' }}: <b>{{ round($sum - (isset($showAverageIncome) && $showAverageIncome ? ( (int)Settings::getSettings()['my_status'] ? (int)Settings::getSettings()['fix_contribution'] : 0 ) : 0)) }}₽</b><br>
    @if (Auth::user()->is_admin)
        @if (isset($showAverageIncome) && $showAverageIncome)
            ...
            @if ((int)Settings::getSettings()['my_status'])
                Фикс. ПФРФ: <b>{{ Settings::getSettings()['fix_contribution'] }}₽</b><br>
            @endif
        @endif
