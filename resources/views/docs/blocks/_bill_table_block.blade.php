<table class="bill">
    ...
    @if ($task->paid_off && !$fullVal)
        <tr>
            <td>{{ 'Предоплата за '.Helper::mbFirstStrToLower(preg_replace('/тка\s/ui','тку ',$task->name)) }}</td>
    ...
    @elseif (count($task->subTasks))
        @foreach($task->subTasks as $subTask)
            <tr>
                <td>{{ $subTask->name }}</td>
                <td class="value">{{ Helper::moneyFormat((int)$subTask->value) }} ₽</td>
            </tr>
        @endforeach
    @else
        <tr>
            <td>{{ $task->name }}</td>
            <td class="value">@include('docs.blocks._task_value_money_format_block', ['billMode' => (isset($billMode) ? $billMode : false), 'fullVal' => true])₽</td>
        </tr>
    @endif
    @if ($useTotal)
        <tr>
